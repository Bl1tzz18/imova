...
public class MarkListingAsRentedHandler(IApplicationDbContext dbContext, IBlobStorageService blobStorageService)
    : IRequestHandler<MarkListingAsRentedCommand, ListingDto?>
{
    public async Task<ListingDto?> Handle(MarkListingAsRentedCommand request, CancellationToken cancellationToken)
    {
        ...
        ListingTransitions.Apply(listing.MarkAsRented);
