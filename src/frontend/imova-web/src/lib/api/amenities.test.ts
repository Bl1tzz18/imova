...
describe("parseCachedAmenities", () => {
  const current = [{ id: "1", key: "sauna", labelRo: "Saună", category: "Leisure", applicablePropertyTypes: ["House"] }];
  ...
  it("rejects the shape without applicable property types, so it gets refetched", () => {
    expect(
      parseCachedAmenities(JSON.stringify([{ id: "1", key: "sauna", labelRo: "Saună", category: "Leisure" }])),
    ).toBeUndefined();
  });
